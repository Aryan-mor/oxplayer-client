...
/// TL identifier (lowerCamel) → Dart PascalCase, e.g. `authenticationCodeTypeSms` → `AuthenticationCodeTypeSms`.
/// Types already PascalCase from the scheme are returned unchanged.
String tlNameToDartClassName(String id) {
  if (id.isEmpty) return id;
  if (!RegExp(r'^[a-z]').hasMatch(id)) return id;
  final withSep = id.replaceAllMapped(
    RegExp(r'([a-z])([A-Z])'),
    (m) => '${m[1]}_${m[2]}',
  );
  return withSep.split('_').map((w) {
    if (w.isEmpty) return w;
    return w[0].toUpperCase() + w.substring(1);
  }).join();
}

late String clientRoot;
late String generatorRoot;
late String tdApiDir;
late File tdApiFile;
...
class DartTdDocumentationGenerator {
  ...
  void dispatchSchemeData(List<String> lines) {
    ...
    for (final line in lines) {
      ...
      if (fieldsRegx.hasMatch(line)) {
        ...
        final dartClass = tlNameToDartClassName(className);
        final dartReturn = tlNameToDartClassName(classReturnType!);
        final rawArgs = classArgs?.trim();
        final parsedArgs = <String, String>{};
        if (rawArgs != null && rawArgs.isNotEmpty) {
          for (final piece in rawArgs.split(' ')) {
            if (!piece.contains(':')) continue;
            final idx = piece.indexOf(':');
            if (idx <= 0 || idx >= piece.length - 1) continue;
            parsedArgs[piece.substring(0, idx)] = piece.substring(idx + 1);
          }
        }

        // store class data and reset class and variables descriptions.
        _objects.add(TlObject(
            dartClass, classDescription, section!,
            variablesDescriptions: variablesDescriptions,
            argsData: parsedArgs,
            returnType: dartReturn));
        classDescription = '';
  ...
  /// write data to file
  void writeToFile() {
    Directory(tdApiDir).createSync(recursive: true);
    File(p.join(tdApiDir, 'object.dart')).writeAsStringSync(
      "part of 'tdapi.dart';\n\n"
    ...
    File(p.join(tdApiDir, 'function.dart')).writeAsStringSync(
      "part of 'tdapi.dart';\n\n"
    ...
    for (final obj in _objects) {
      final temple =
          File(p.join(generatorRoot, 'main_class.tmpl')).readAsStringSync();
  ...
  """
    ...
    final convertorTemple =
        File(p.join(generatorRoot, 'convertor.tmpl')).readAsStringSync();
    var cases = '';
...
Future<void> main(List<String> args) async {
  ...
  generatorRoot = p.join(clientRoot, 'tool', 'generator');
  tdApiDir = p.join(clientRoot, 'lib', 'td_api_generated');
  tdApiFile = File(p.join(tdApiDir, 'tdapi.dart'));

  final schemeFile = File(p.join(generatorRoot, 'data', 'td_api.tl'));
  schemeFile.parent.createSync(recursive: true);
  if (args.contains('--fetch') || !schemeFile.existsSync()) {
    await _fetchOfficialScheme(schemeFile);
  }

  final gen = DartTdDocumentationGenerator()..schemePath = schemeFile.path;
  gen.generate();

  File(p.join(clientRoot, 'lib', 'td_api_generated', 'td_api.dart'))
      .writeAsStringSync("export 'tdapi.dart';\n");
}

Future<void> _fetchOfficialScheme(File out) async {
  final verPath = p.join(clientRoot, 'tool', 'tdlib', 'TD_VERSION.json');
  final ver = jsonDecode(File(verPath).readAsStringSync()) as Map<String, dynamic>;
  final url = ver['td_api_tl_raw_url'] as String? ?? '';
  if (url.isEmpty) {
    stderr.writeln('TD_VERSION.json missing td_api_tl_raw_url');
    exit(1);
  }
  stderr.writeln('Fetching scheme: $url');
  final res = await http.get(Uri.parse(url));
  if (res.statusCode != 200) {
    stderr.writeln('HTTP ${res.statusCode}');
    exit(1);
  }
  out.writeAsBytesSync(res.bodyBytes);
}
...
class TlObjectArg {
  ...
  static String getType(dynamic raw, {String prefix = 'TYPE'}) {
    String dartType;
    final typeStr = raw?.toString() ?? '';
    if (builtInTypes.contains(typeStr)) {
      dartType = getBuiltInDartType(typeStr);
      ...
      var mapped = tlNameToDartClassName(typeStr);
      if (mapped == 'Error') mapped = 'TdError';
      dartType = mapped;
    }
